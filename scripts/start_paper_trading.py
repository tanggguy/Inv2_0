...
def check_environment():
    ...
    if not Path(".env").exists():
        errors.append(
            "Fichier .env non trouvé. Copiez .env.example vers .env et configurez-le."
        )
    else:
    ...
    try:
        ...
        errors.append(
            "alpaca-trade-api non installé. Exécutez: pip install alpaca-trade-api"
        )
...
def display_config():
    ...
    # Stratégies actives
    active_strategies = [
        s for s in PAPER_TRADING_CONFIG["strategies"] if s.get("enabled", True)
    ]
    ...
    print(f"\n📱 NOTIFICATIONS:")
    print(
        f"  • Telegram:            {'✅ Activé' if notif_config['telegram_enabled'] else '❌ Désactivé'}"
    )
    print(
        f"  • Email:               {'✅ Activé' if notif_config['email_enabled'] else '❌ Désactivé'}"
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--config", action="store_true", help="Afficher la configuration et quitter"
    )

    parser.add_argument(
        "--test-telegram", action="store_true", help="Tester les notifications Telegram"
    )
