            ...
            if 0 <= idx < len(presets):
                ...
                if strategy_name:
                    config = config_manager.get_config_for_strategy(
                        strategy_name=strategy_name, preset_name=preset_name
                    )
...
def customize_config(config, strategy_name):
    ...
    new_symbols = input(
        "Nouveaux symboles (séparés par espace) [Enter pour garder]: "
    ).strip()
    if new_symbols:
...
def main():
    ...
    try:
        ...
        is_valid, warnings = config_manager.validate_strategy_params(
            strategy_class, config.get("param_grid", {})
        )
        ...
        if warnings:
            ...
            confirm = (
                input("\nContinuer malgré les avertissements ? (o/n) [n]: ")
                .strip()
                .lower()
            )
            if confirm != "o":
