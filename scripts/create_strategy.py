...
def configure_trailing_stop():
    ...
    use_trailing = input("Utiliser un Trailing Stop? (o/n) [o]: ").strip().lower() or 'o'
...
def generate_strategy_code(name, template, stop_loss, take_profit, trailing_stop):
    ...
    if trailing_stop:
        ...
        stop_params.append(f"('trailing_activation_pct', {trailing_stop['activation_pct']}),")
...
def create_strategy_interactive():
    ...
    try:
        ...
        code = generate_strategy_code(name, template, stop_loss, take_profit, trailing_stop)
