...
class YAMLOptunaRunner:
    ...
    def __init__(self, yaml_path: str, profile: Optional[str] = None, verbose: bool = False):
        """
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if self.profile:
            ...
            logger.info(f"✅ Application du profil '{self.profile}': {profile_config.get('description', '')}")
    ...
    def _validate_config(self, config: Dict) -> None:
        ...
        # Valider period
        if 'start' not in config['config']['period'] or 'end' not in config['config']['period']:
    ...
    def load_strategy_class(self, class_path: str):
        ...
        try:
            ...
            raise ImportError(f"❌ Impossible de charger la stratégie '{class_path}': {e}")
    ...
    def _print_summary(self, strategy_name: str, config: Dict) -> None:
        ...
        logger.info(f"   Période: {config['period']['start']} → {config['period']['end']}")
    ...
    def _print_results(self, results: Dict) -> None:
        ...
        logger.info(f"📊 Visualiser avec: optuna-dashboard sqlite:///optimization/optuna_studies/optuna.db")
        logger.info("="*80 + "\n")
