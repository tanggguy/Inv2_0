...
class WalkForwardOptimizer:
    ...
    def __init__(self, strategy_class, symbols, 
    ...
    def generate_periods(self):
        ...
        while current_date + relativedelta(months=self.in_sample_months + self.out_sample_months) <= self.end_date:
    ...
    def _analyze_walk_forward(self):
        ...
        print("\n🎯 PERFORMANCE MOYENNE:")
        print(f"   In-Sample Sharpe:   {df['in_sample_sharpe'].mean():.2f} (±{df['in_sample_sharpe'].std():.2f})")
        print(f"   Out-Sample Sharpe:  {df['out_sample_sharpe'].mean():.2f} (±{df['out_sample_sharpe'].std():.2f})")
        ...
        summary = df[['period', 'in_sample_sharpe', 'out_sample_sharpe', 
                      'in_sample_return', 'out_sample_return', 
                      'out_sample_trades', 'degradation']]
...
╚═══════════════════════════════════════════════════════════════════════╝
    ...
    for i, p in enumerate(periods, 1):
        print(f"   {i}. In: {p['in_sample'][0]} → {p['in_sample'][1]} | "
