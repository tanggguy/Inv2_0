...
class MetricsValidator:
    ...
    def _validate_portfolio_values(self, metrics: Dict) -> Dict:
        ...
        if initial <= self.MIN_CAPITAL:
            ...
            if self.verbose:
                logger.warning(
                    f"Correction: initial_value = {metrics['initial_value']}"
                )

        if final < 0:
            self.validation_issues.append(f"Valeur finale négative: {final}")
    ...
    def _recalculate_return(self, metrics: Dict) -> Dict:
        ...
        try:
            ...
            if not np.isfinite(return_pct):
                self.validation_issues.append(f"Return non-fini détecté: {return_pct}")
                return_pct = 0.0
            ...
            self.validation_issues.append(f"Erreur dans le calcul du return: {e}")
            metrics["total_return"] = 0.0
    ...
    def _validate_drawdown(self, metrics: Dict) -> Dict:
        ...
        if not np.isfinite(drawdown):
            self.validation_issues.append(f"Drawdown non-fini détecté: {drawdown}")
            drawdown = 0.0
    ...
    def _validate_sharpe(self, metrics: Dict) -> Dict:
        ...
        if not np.isfinite(sharpe):
            self.validation_issues.append(f"Sharpe ratio non-fini détecté: {sharpe}")
            sharpe = 0.0
...
if __name__ == "__main__":
    ...
    problematic_metrics = {
        ...
        "final_value": 150000,
        "total_return": 1.5e100,  # Valeur aberrante
        "max_drawdown": -99999,
        "total_trades": 2,
        "won_trades": 1,
        "lost_trades": 1,
