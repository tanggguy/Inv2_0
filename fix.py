...
class RSIDiagnostic(bt.Strategy):
    ...
    def next(self):
        ...
        if rsi_val < self.params.rsi_oversold:
            ...
            if not self.position:
                ...
                if self.params.printlog:
                    print(f"{self.datas[0].datetime.date(0)} | 🟢 ACHAT | RSI={rsi_val:.1f} < {self.params.rsi_oversold}")
        ...
        if rsi_val > self.params.rsi_overbought:
            ...
            if self.position:
                self.sell_signals += 1
                if self.params.printlog:
                    print(f"{self.datas[0].datetime.date(0)} | 🔴 VENTE | RSI={rsi_val:.1f} > {self.params.rsi_overbought}")
    ...
    def stop(self):
        ...
        if self.rsi_ready_days > 0:
            ...
            print(f"   Jours RSI < {self.params.rsi_oversold}: {self.oversold_days} ({self.oversold_days/self.rsi_ready_days*100:.1f}%)")
            print(f"   Jours RSI > {self.params.rsi_overbought}: {self.overbought_days} ({self.overbought_days/self.rsi_ready_days*100:.1f}%)")
            print(f"   Signaux d'achat générés: {self.buy_signals}")
            print(f"   Signaux de vente générés: {self.sell_signals}")
            
            print(f"\n💡 ANALYSE:")
            if self.buy_signals < 5:
                print(f"   ❌ PROBLÈME: Seuil RSI oversold ({self.params.rsi_oversold}) trop BAS")
                print(f"   → RSI descend rarement en-dessous de {self.params.rsi_oversold}")
                print(f"   → Seulement {self.oversold_days} jours sur {self.rsi_ready_days} ({self.oversold_days/self.rsi_ready_days*100:.1f}%)")
                print()
                print(f"   💊 SOLUTION:")
                print(f"      • Augmenter rsi_oversold à 30-35")
                print(f"      • Cela générera ~{int(self.rsi_ready_days * 0.1)} signaux (10% des jours)")
            
            if self.sell_signals < 5:
                print(f"   ❌ PROBLÈME: Seuil RSI overbought ({self.params.rsi_overbought}) trop HAUT")
                print(f"   → RSI monte rarement au-dessus de {self.params.rsi_overbought}")
                print(f"   → Seulement {self.overbought_days} jours sur {self.rsi_ready_days} ({self.overbought_days/self.rsi_ready_days*100:.1f}%)")
                print()
                print(f"   💊 SOLUTION:")
...
def analyze_rsi_thresholds(symbol="AAPL", start="2021-01-01", end="2025-01-01"):
    ...
    for r in results:
        ...
        print(f"{r['config']:<20} {r['buy_signals']:<6} {r['sell_signals']:<6} "
...
if __name__ == "__main__":
    ...
    parser.add_argument('--compare', action='store_true', help='Comparer plusieurs configs')
