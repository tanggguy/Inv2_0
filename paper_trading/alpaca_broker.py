...
class AlpacaBroker(BrokerBase):
    ...
    def _sync_account(self):
        ...
        try:
            ...
            if account:
                ...
                logger.info(
                    f"Compte synchronisé - Cash: ${self._cash:.2f}, Value: ${self._value:.2f}"
                )
            ...
            for symbol, position in positions.items():
                ...
                logger.info(
                    f"Position {symbol}: {position.qty} @ ${position.avg_entry_price}"
                )
    ...
    def _submit_order(
        self, owner, data, size, side, price=None, exectype=None, **kwargs
    ):
        ...
        try:
            ...
            if alpaca_order:
                ...
                self.notify(order)
                logger.info(
                    f"Ordre soumis: {alpaca_side} {size} {symbol} @ {order_type}"
                )
        ...
        try:
            ...
            with self._lock:
                ...
                if trade.event == "fill" or trade.event == "partial_fill":
                    # Ordre exécuté
                    order.status = (
                        Order.Completed if trade.event == "fill" else Order.Partial
                    )
                    order.executed.size = float(trade.order.filled_qty)
                    order.executed.price = (
                        float(trade.order.filled_avg_price)
                        if trade.order.filled_avg_price
                        else 0
                    )
                    ...
                    logger.info(
                        f"Trade exécuté: {trade.order.side} {trade.order.filled_qty} {symbol} @ ${trade.order.filled_avg_price}"
                    )

                elif trade.event == "rejected" or trade.event == "canceled":
                    # Ordre rejeté ou annulé
                    order.status = (
                        Order.Rejected if trade.event == "rejected" else Order.Canceled
                    )
