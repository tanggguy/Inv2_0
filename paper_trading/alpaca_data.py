...
class AlpacaData(DataBase):
    ...
    def _load_historical(self):
        ...
        try:
            ...
            bars = self.store.get_bars(
                ...
                timeframe=timeframe,
                limit=self.params.backfill_days * 24 if timeframe == '1H' else self.params.backfill_days
            )
            ...
            if bars:
                ...
                logger.info(f"{len(bars)} barres historiques chargées pour {self.params.symbol}")
            else:
    ...
    def _add_bar(self, bar):
        ...
        try:
            with self._lock:
                self._data_queue.put(bar)
                logger.debug(f"Barre ajoutée pour {self.params.symbol}: {bar['datetime']}")
        except Exception as e:
