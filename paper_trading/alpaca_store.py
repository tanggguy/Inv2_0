...
class AlpacaStore(with_metaclass(MetaSingleton, object)):
    ...
    def _init_api(self):
        ...
        try:
            ...
            logger.info(
                f"Connecté à Alpaca - Cash: ${account.cash}, Buying Power: ${account.buying_power}"
            )
    ...
    def _handle_trade_update(self, trade):
        ...
        logger.info(
            f"Trade update: {trade.event} pour {trade.order.symbol} - Status: {trade.order.status}"
        )
    ...
    def get_bars(self, symbol, timeframe="1H", limit=100):
        ...
        try:
            ...
            tf = (
                TimeFrame(1, TimeFrameUnit.Hour) if timeframe == "1H" else TimeFrame.Day
            )
