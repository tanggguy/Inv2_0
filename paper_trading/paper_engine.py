...
class PaperTradingEngine:
    ...
    def initialize(self):
        ...
        try:
            ...
            if previous_state:
                logger.info(
                    f"   ✓ État précédent restauré ({previous_state['timestamp']})"
                )
            else:
            ...
            logger.info(
                f"   ✓ Circuit breaker activé (Capital: ${initial_capital:,.2f})"
            )
            ...
            self.strategy_runner = MultiStrategyRunner(
                self.config, self.store, self.circuit_breaker, self.portfolio_manager
            )
            self.strategy_runner.initialize()

            # Afficher les stratégies actives
            active_strategies = [
                s["name"] for s in self.config["strategies"] if s.get("enabled", True)
            ]
            logger.info(
                f"   ✓ {len(active_strategies)} stratégies prêtes: {', '.join(active_strategies)}"
            )
            ...
            while self.running:
                ...
                if self.config["notifications"].get("telegram_daily_summary"):
                    ...
                    summary_time = datetime.strptime(
                        self.config["notifications"]["telegram_summary_time"], "%H:%M"
                    ).time()

                    if (
                        now.time() >= summary_time
                        and now.date() != last_summary_time.date()
                    ):
    ...
    def _check_performance(self):
        ...
        try:
            ...
            self.portfolio_manager.update_state(
                "account",
                {
            ...
            # Log périodique
            logger.info(
                f"Performance Check - Value: ${current_value:,.2f} | "
...
🎯 **Stratégies**"""
    ...
    def _send_final_summary(self):
        ...
        try:
            ...
            strategy_perf = (
                self.strategy_runner.get_performance() if self.strategy_runner else {}
            )
