...
class CircuitBreaker:
    ...
    def start(self, initial_capital):
        ...
        if self.config["enabled"]:
            ...
            logger.info(
                f"Circuit breaker démarré - Capital initial: ${initial_capital:.2f}"
            )
        ...
        if drawdown > self.config["max_drawdown_pct"]:
            if not self.breakers_status["drawdown"]["triggered"]:
                ...
                self._trigger_breaker(
                    "drawdown",
                    f"Drawdown de {drawdown:.1%} > {self.config['max_drawdown_pct']:.1%}",
                )
            return True
    ...
    def _check_daily_loss(self, current_value):
        ...
        daily_loss = (
            (self.daily_starting_value - current_value) / self.daily_starting_value
        ...
        if daily_loss > self.config["max_daily_loss_pct"]:
            if not self.breakers_status["daily_loss"]["triggered"]:
                ...
                self._trigger_breaker(
                    "daily_loss",
                    f"Perte quotidienne de {daily_loss:.1%} > {self.config['max_daily_loss_pct']:.1%}",
                )
            return True
            ...
            if pnl is not None and pnl < 0:
                ...
                if self.consecutive_losses >= self.config["max_consecutive_losses"]:
                    if not self.breakers_status["consecutive_losses"]["triggered"]:
                        ...
                        self.breakers_status["consecutive_losses"][
                            "timestamp"
                        ] = datetime.now()
                        self._trigger_breaker(
                            "consecutive_losses",
        ...
        with self._lock:
            ...
            for name, breaker in self.breakers_status.items():
                status["breakers"][name] = {
                    ...
                    "value": breaker["value"],
                    "timestamp": (
                        breaker["timestamp"].isoformat()
                        if breaker["timestamp"]
                        else None
                    ),
                }
