...
def test_grid_search(config):
    ...
    optimizer = UnifiedOptimizer(
        MaRSI, config, optimization_type="grid_search", verbose=True
    )
...
def test_walk_forward():
    ...
    optimizer = UnifiedOptimizer(
        MaRSI, config, optimization_type="walk_forward", verbose=True
    )
...
def test_storage():
    ...
    if runs:
        ...
        if loaded:
            ...
            print(
                f"     Best Sharpe: {loaded['summary']['best_params'].get('sharpe', 0):.2f}"
            )
...
def test_custom_config():
    ...
    custom = config_manager.create_custom(
        ...
        end_date="2023-12-31",
        param_grid={"ma_period": [15, 25], "rsi_period": [14]},
        capital=50000,
        name="Test Custom",
    ...
    merged = config_manager.merge_configs(
        "quick", {"symbols": ["MSFT"], "capital": 150000}
    )
