...
def display_comparison_table(results):
    ...
    df.columns = [
        'Stratégie', 'Symboles', 'Capital Final', 'Rendement', 
        'Sharpe', 'Max DD', 'Trades', 'Win Rate', 'Gain Moy', 'Perte Moy'
    ]
...
def identify_best_strategy(results):
    ...
    print("="*70)
    print(f"📈 Meilleur Rendement:     {best_return['strategy']:<25} {best_return['total_return']:.2f}%")
    print(f"⭐ Meilleur Sharpe Ratio:  {best_sharpe['strategy']:<25} {best_sharpe['sharpe_ratio']:.2f}")
    print(f"🎯 Meilleur Win Rate:      {best_winrate['strategy']:<25} {best_winrate['win_rate']:.2f}%")
    print(f"🛡️  Plus Faible Drawdown:  {min_drawdown['strategy']:<25} {min_drawdown['max_drawdown']:.2f}%")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Testeur et comparateur de stratégies"
    )
    ...
    parser.add_argument(
        '--quick',
        action='store_true',
        help='Test rapide (période courte)'
    )
