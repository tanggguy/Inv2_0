...
class TestAlpacaStore:
    ...
    def test_submit_order(self, mock_rest):
        ...
        store = AlpacaStore(api_key="test_key", secret_key="test_secret")
...
class TestMultiStrategyRunner:
    ...
    def test_strategy_instance_init(self):
        ...
        instance = StrategyInstance(
            name="TestStrategy", config=config, store=mock_store
        )
...
class TestIntegration:
    ...
    @pytest.mark.skipif(
        not os.getenv("ALPACA_API_KEY"), reason="Clés API non disponibles"
    )
    ...
    @pytest.mark.skipif(
        not os.getenv("TELEGRAM_BOT_TOKEN"), reason="Token Telegram non disponible"
    )
