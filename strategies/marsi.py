...
class MaRSI(BaseAdvancedStrategy):
    ...
    def __init__(self):
        ...
        self.ma_fast = bt.indicators.SMA(
            self.datas[0].close, 
            period=self.params.fast_ma_period
        )
        self.ma_slow = bt.indicators.SMA(
            self.datas[0].close, 
            period=self.params.slow_ma_period
        )
        ...
        self.log("=" * 70)
        self.log(f"MA Rapide: {self.params.fast_ma_period} | MA Lente: {self.params.slow_ma_period}")
        self.log(f"RSI: {self.params.rsi_period} (Survente: {self.params.rsi_oversold}, Surachat: {self.params.rsi_overbought})")
        self.log(f"Filtres actifs: Tendance={self.params.use_trend_filter}, Volume={self.params.use_volume_filter}")
        self.log(f"Gestion risque: Stop ATR {self.params.stop_loss_atr_mult}x, TP {self.params.take_profit_pct*100}%")
        self.log("=" * 70)
    ...
    def calculate_position_size(self):
        ...
        self.log(f'│ Risque par trade: {risk_amount:.2f} € ({self.params.risk_per_trade*100}%)')
        self.log(f'│ ATR: {atr_value:.2f}')
        self.log(f'│ Stop distance: {stop_distance:.2f} ({self.params.stop_loss_atr_mult}x ATR)')
        ...
        self.log(f'│ Valeur position: {final_size * current_price:.2f} € ({(final_size * current_price / capital)*100:.1f}%)')
        self.log('└────────────────────────────────────────────')
    ...
    def validate_entry_signal(self):
        ...
        if self.params.use_trend_filter:
            ...
            self.log(f"│    Prix: {current_price:.2f} | EMA200: {self.ema_trend[0]:.2f}")
        ...
        if self.params.use_volume_filter:
            ...
            self.log(f"│    Ratio: {volume_ratio:.2f}x (min: {self.params.min_volume_ratio}x)")
    ...
    def next(self):
        ...
        if self.crossover > 0:
            ...
            self.log(f'🔔 CROSSOVER HAUSSIER détecté - Fenêtre active {self.params.crossover_window} barres')
        ...
        if self.last_crossover_direction != 0:
            ...
            if  self.bars_since_crossover <= self.params.crossover_window:
                ...
                self.log(f"📅 Fenêtre active: Jour {self.bars_since_crossover}/{self.params.crossover_window} (reste {remaining} barres)")
        ...
        if not self.position:
            ...
            in_crossover_window = (
                self.last_crossover_direction == 1 and 
                self.bars_since_crossover <= self.params.crossover_window
            )
            ...
            elif self.last_crossover_direction == 1 and self.bars_since_crossover > self.params.crossover_window:
    ...
    def stop(self):
        ...
        self.log(f"║ Taux de Filtrage: {(self.filtered_signals/self.signal_count*100) if self.signal_count > 0 else 0:.1f}%")
        self.log("╚═══════════════════════════════════════════════════╝")
