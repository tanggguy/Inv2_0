...
class BreakoutATRStop(BaseAdvancedStrategy):
    ...
    def __init__(self):
        ...
        self.highest = bt.indicators.Highest(
            self.datas[0].high, period=self.params.breakout_period
        )

        self.lowest = bt.indicators.Lowest(
            self.datas[0].low, period=self.params.breakout_period
        )

        # Volume moyen
        if self.params.volume_confirmation:
            self.avg_volume = bt.indicators.SMA(self.datas[0].volume, period=20)
...
class MomentumMultipleStops(BaseAdvancedStrategy):
    ...
    def __init__(self):
        ...
        self.roc = bt.indicators.ROC(self.datas[0].close, period=self.params.roc_period)
        ...
        if self.position and self.entry_price:
            ...
            if gain_pct >= self.params.aggressive_trigger:
                if self.highest_price > current_price:
                    aggressive_stop = self.highest_price * (
                        1 - self.params.aggressive_trailing_pct
                    )
