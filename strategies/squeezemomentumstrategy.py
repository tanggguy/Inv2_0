...
class SqueezeMomentumStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        self.atr_sma = bt.indicators.SMA(self.atr, period=self.params.min_squeeze_days)
    ...
    def validate_squeeze_quality(self):
        ...
        if self.params.use_squeeze_volume_factor:
            avg_volume_ratio = sum(self.squeeze_volume_history) / len(
                self.squeeze_volume_history
            )
        ...
        if self.params.use_squeeze_atr_factor:
            if len(self.squeeze_atr_history) > 0:
                avg_atr_ratio = sum(self.squeeze_atr_history) / len(
                    self.squeeze_atr_history
                )
    ...
    def check_market_context(self):
        ...
        if self.params.use_trend_filter:
            if self.data.close[0] < self.sma_200[0]:
                return (
                    False,
                    f"Prix sous SMA 200 ({self.data.close[0]:.2f} < {self.sma_200[0]:.2f})",
                )
        ...
        if self.params.use_rsi_filter:
            if self.rsi[0] > self.params.rsi_max:
                return (
                    False,
                    f"RSI surachat ({self.rsi[0]:.2f} > {self.params.rsi_max})",
                )
    ...
    def check_breakout_conditions(self):
        ...
        if close <= bb_upper:
            return (
                False,
                f"Prix ne dépasse pas BB Upper ({close:.2f} <= {bb_upper:.2f})",
            )
        ...
        if self.params.use_min_candle_body_pct:
            ...
            if candle_range > 0:
                ...
                if body_pct < self.params.min_candle_body_pct:
                    return (
                        False,
                        f"Corps de bougie trop petit ({body_pct*100:.1f}% < {self.params.min_candle_body_pct*100:.1f}%)",
                    )
        ...
        if volume_ratio < self.params.breakout_volume_mult:
            return (
                False,
                f"Volume insuffisant ({volume_ratio:.2f}× < {self.params.breakout_volume_mult}×)",
            )
        ...
        if self.params.use_breakout_adx_min:
            if self.adx[0] < self.params.breakout_adx_min:
                return (
                    False,
                    f"ADX trop bas ({self.adx[0]:.2f} < {self.params.breakout_adx_min})",
                )
        ...
        if self.params.require_adx_rising:
            if self.adx[0] <= self.adx[-1]:
                return (
                    False,
                    f"ADX non croissant ({self.adx[0]:.2f} <= {self.adx[-1]:.2f})",
                )
    ...
    def calculate_position_size(self, entry_price, stop_loss):
        ...
        if required_capital > available_cash:
            ...
            self.log(f"⚠️  Capital insuffisant. Taille ajustée: {size}", level="WARNING")
    ...
    def update_trailing_stop(self):
        ...
        activation_threshold = (
            self.last_squeeze_height * self.params.trailing_stop_activation
        )
    ...
    def check_exit_conditions(self):
        ...
        if current_price >= self.target_price:
            return (
                True,
                f"TARGET atteint @ ${current_price:.2f} (Target: ${self.target_price:.2f})",
            )
        ...
        if self.trailing_stop_active and current_price <= self.trailing_stop:
            return (
                True,
                f"TRAILING STOP @ ${current_price:.2f} (Trailing: ${self.trailing_stop:.2f})",
            )
        ...
        if self.params.use_breakout_adx_min:
            if (
                self.adx[0] < self.params.breakout_adx_min * 0.7
            ):  # ADX tombe sous 70% du minimum
    ...
    def next(self):
        ...
        if self.is_squeeze_active():
            if not self.in_squeeze:
                ...
                self.squeeze_volume_history.append(
        ...
        if not self.order and not self.position:
            ...
            if breakout_valid:
                ...
                size = self.calculate_position_size(self.entry_price, self.stop_loss)

                if size > 0:
                    risk_reward = (self.target_price - self.entry_price) / (
                ...
                # Breakout invalide
                if self.params.debug_mode:
                    self.log(f"❌ Breakout rejeté: {breakout_reason}", level="DEBUG")
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                profit = (
                    order.executed.price - self.entry_price if self.entry_price else 0
                )
                profit_pct = (
                    (profit / self.entry_price * 100) if self.entry_price else 0
                )
    ...
    def notify_trade(self, trade):
        ...
        days_in_trade = (
            (self.data.datetime.date(0) - self.entry_date).days
            if self.entry_date
            else 0
        )
