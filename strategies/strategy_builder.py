...
class StrategyBuilder:
    ...
    def add_indicator(self, indicator_type, **params):
        ...
        Returns:
            self pour chaînage
        """
        self.indicators.append({"type": indicator_type, "params": params})
        return self
    ...
    def build(self):
        ...
        if self.take_profit_config:
            ...
            if self.take_profit_config.get("percentage"):
                params.append(
                    ("take_profit_pct", self.take_profit_config["percentage"])
                )
            if self.take_profit_config.get("risk_reward"):
                params.append(
                    ("risk_reward_ratio", self.take_profit_config["risk_reward"])
                )

        if self.trailing_stop_config:
            params.append(("use_trailing_stop", True))
            params.append(
                ("trailing_stop_pct", self.trailing_stop_config["percentage"])
            )
            params.append(
                ("trailing_activation_pct", self.trailing_stop_config["activation"])
            )
                ...
                for ind_config in self.indicators:
                    ...
                    if ind_type == "SMA":
                        self._indicators["sma"] = bt.indicators.SMA(
                            self.datas[0].close, period=ind_params.get("period", 20)
                        )
                    elif ind_type == "EMA":
                        ...
                        self._indicators["rsi"] = bt.indicators.RSI(
                            self.datas[0].close, period=ind_params.get("period", 14)
                        )
                    elif ind_type == "MACD":
                        ...
                        self._indicators["atr"] = bt.indicators.ATR(
                            self.datas[0], period=ind_params.get("period", 14)
                        )
            ...
            def next(self):
                ...
                if not self.position:
                    ...
                    if all(cond(self) for cond in self.entry_conditions):
                        size = int(
                            (self.broker.getcash() * 0.95) / self.datas[0].close[0]
                        )
