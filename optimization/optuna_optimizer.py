...
class OptunaOptimizer:
    ...
    def _create_sampler(self, sampler_type: str):
        ...
        if sampler_type not in samplers:
            if self.logger:
                self.logger.warning(f"Sampler '{sampler_type}' inconnu, utilisation de 'tpe'")
            sampler_type = 'tpe'
    ...
    def _create_pruner(self, pruner_type: str):
        ...
        if pruner_type not in pruners:
            if self.logger:
                self.logger.warning(f"Pruner '{pruner_type}' inconnu, utilisation de 'median'")
            pruner_type = 'median'
    ...
    def optimize(
        ...
        def _progress_callback(study, trial):
            if progress_callback:
                ...
                if trial.number > 0:
                    avg_duration = sum(t.duration.total_seconds() for t in study.trials if t.duration) / len([t for t in study.trials if t.duration])
        ...
        try:
            ...
            if self.logger:
                ...
                self.logger.info(f"Completed trials: {len([t for t in self.study.trials if t.state == optuna.trial.TrialState.COMPLETE])}")
                self.logger.info(f"Pruned trials: {len([t for t in self.study.trials if t.state == optuna.trial.TrialState.PRUNED])}")
                self.logger.info(f"Failed trials: {len([t for t in self.study.trials if t.state == optuna.trial.TrialState.FAIL])}")
