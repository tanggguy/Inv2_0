...
class OptimizationConfig:
    ...
    def get_preset(self, name: str) -> Optional[Dict]:
        ...
        if preset:
            _get_logger().info(
                f"Preset '{name}' chargé: {preset.get('description', '')}"
            )
            return preset.copy()
        else:
    ...
    def get_config_for_strategy(
        ...
        if not config:
            raise ValueError(
                f"Preset '{preset_name}' introuvable. Disponibles: {self.list_presets()}"
            )
    ...
    def validate_strategy_params(
        self, strategy_class, param_grid: Dict
    ) -> tuple[bool, List[str]]:
        """
    ...
    def create_custom(
        ...
        config = {
            "name": name,
            "description": description
            or f"Config custom créée le {datetime.now().strftime('%Y-%m-%d')}",
            "symbols": symbols,
    ...
    def validate_config(
        self, config: Dict, strategy_class=None
    ) -> tuple[bool, List[str]]:
        """
...
def load_preset(name: str) -> Dict:
    ...
    if not preset:
        raise ValueError(
            f"Preset '{name}' introuvable. Disponibles: {config_manager.list_presets()}"
        )
    return preset
...
if __name__ == "__main__":
    ...
    custom = config_manager.create_custom(
        ...
        end_date="2024-01-01",
        param_grid={"ma_period": [10, 20, 30], "rsi_period": [14, 21]},
        name="Test Custom",
    ...
    merged = config_manager.merge_configs(
        "quick",
        {"symbols": ["MSFT", "GOOGL"], "param_grid": {"ma_period": [15, 25, 35]}},
    )
