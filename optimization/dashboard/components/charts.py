...
def create_equity_curve(results_df: pd.DataFrame, run_id: str = None) -> go.Figure:
    ...
    if 'pnl' in results_df.columns:
        ...
        fig.add_trace(go.Scatter(
            ...
            y=results_df['equity'] if 'equity' in results_df else results_df.iloc[:, 0],
            mode='lines',
            name='Equity',
            line=dict(color='#667eea', width=2),
            fill='tozeroy',
...
def create_heatmap(results_df: pd.DataFrame, x_param: str, y_param: str, metric: str = 'sharpe') -> go.Figure:
    """
...
def create_distribution_chart(results_df: pd.DataFrame, metric: str = 'sharpe') -> go.Figure:
    """
...
def create_scatter_plot(results_df: pd.DataFrame, 
                       x_metric: str = 'return', 
                       y_metric: str = 'sharpe',
...
def create_parameter_impact_chart(results_df: pd.DataFrame, 
                                  param: str, 
    ...
    grouped = results_df.groupby(param)[metric].agg(['mean', 'std', 'count']).reset_index()
