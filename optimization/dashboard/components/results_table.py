...
def display_runs_table(runs: List[Dict], 
                       selectable: bool = False,
                       show_actions: bool = True) -> List[str]:
    """
    ...
    # Colonnes à afficher
    display_cols = ['run_id', 'strategy', 'type', 'best_sharpe', 'best_return', 
                   'total_combos', 'symbols', 'timestamp']
    ...
    if 'symbols' in df_display.columns:
        df_display['symbols'] = df_display['symbols'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
