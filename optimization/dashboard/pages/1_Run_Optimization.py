...
from dashboard.components.metrics import display_metric_cards, display_detailed_metrics, display_parameters_card
...
if run_button and strategy_class and config and opt_type:
    ...
    if opt_type == "optuna":
        st.info("ℹ️ Optuna: Mode séquentiel activé pour compatibilité avec Streamlit. "
        ...
        if results and 'best' in results:
            ...
            if opt_type == "optuna" and 'param_importance' in results:
                ...
                if importance:
                    ...
                    st.info("💡 Les paramètres avec une importance élevée ont le plus d'impact sur la performance.")
...
# Footer
st.divider()
st.caption("💡 Astuce: Pour des optimisations longues avec parallélisation, utilisez le script CLI.")