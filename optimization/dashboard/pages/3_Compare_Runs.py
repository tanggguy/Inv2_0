...
with col2:
    ...
    if st.button("🏆 Top 3 Sharpe", use_container_width=True):
        top_runs = sorted(all_runs, key=lambda x: x.get('best_sharpe', 0), reverse=True)[:3]
        selected_run_ids = [r['run_id'] for r in top_runs]
        st.rerun()
    
    if st.button("💰 Top 3 Return", use_container_width=True):
        top_runs = sorted(all_runs, key=lambda x: x.get('best_return', 0), reverse=True)[:3]
        selected_run_ids = [r['run_id'] for r in top_runs]
        st.rerun()
...
with tab3:
    ...
    st.success(f"🎯 **Recommandation:** Le run **{best_run['Run']}** obtient le meilleur score global ({best_run['Score']:.2f})")
...
with col1:
    if st.button("🔬 Analyser le meilleur", use_container_width=True):
        ...
        full_id = next((r['run_id'] for r in all_runs if r['run_id'].startswith(best_run_id[:20])), None)
        if full_id:
