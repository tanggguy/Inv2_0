...
class ResultsStorage:
    ...
    def _init_history_file(self):
        ...
        initial_data = {
            "runs": [],
            "metadata": {"created": datetime.now().isoformat(), "total_runs": 0},
        }
    ...
    def _create_summary(
        self, config: Dict, results: Dict, optimization_type: str
    ) -> Dict:
    ...
    def list_runs(self, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        try:
            ...
            if "strategy" in filters:
                strategies = (
                    filters["strategy"]
                    if isinstance(filters["strategy"], list)
                    else [filters["strategy"]]
                )
            ...
            if "min_sharpe" in filters:
                filtered = [
                    r
                    for r in filtered
                    if r.get("best_sharpe", 0) >= filters["min_sharpe"]
                ]
            ...
            if "max_sharpe" in filters:
                filtered = [
                    r
                    for r in filtered
                    if r.get("best_sharpe", 0) <= filters["max_sharpe"]
                ]
            ...
            if "symbols" in filters:
                ...
                filtered = [
                    r
                    for r in filtered
                    if any(s in r.get("symbols", []) for s in symbols_filter)
                ]
            ...
            if "end_date" in filters:
                ...
                filtered = [
                    r for r in filtered if datetime.fromisoformat(r["timestamp"]) <= end
                ]
    ...
    def get_best_run(
        self, strategy: Optional[str] = None, metric: str = "sharpe"
    ) -> Optional[Dict]:
        """
        ...
        logger.info(
            f"Meilleur run: {best['run_id']} ({metric}={best.get(metric_key, 0):.2f})"
        )
        return best
...
if __name__ == "__main__":
    ...
    for run in runs:
        print(
            f"  • {run['run_id']} - {run['strategy']} - Sharpe: {run['best_sharpe']:.2f}"
        )
